...
LYRICS:\n${lyrics}\n\nINSPIRATION:\n${inspiration}`;
    ...
    let song;
    try { song = JSON.parse(content); } catch { return res.status(502).json({ error: 'OpenAI returned invalid song JSON.' }); }

    if (!song.title || !song.key || !song.sections?.length) {
    ...
    if (error?.name === 'AbortError') {
      return res.status(504).json({ error: 'OpenAI took too long to respond. Please try again.' });
    }
