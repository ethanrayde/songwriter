export default async function handler(req, res) {
  ...
  try {
    ...
    // If the songwriter explicitly gives a tempo, that is a hard constraint—not a suggestion.
    const bpmMatch = inspiration.match(/(?:\b(?:around|about|at|near|roughly)?\s*)(\d{2,3})\s*(?:bpm|beats?\s*(?:per\s*minute)?)/i);
    const requestedBpm = bpmMatch ? Number(bpmMatch[1]) : null;
    const bpmInstruction = requestedBpm
      ? `The songwriter explicitly requested ${requestedBpm} BPM. You MUST return bpm exactly ${requestedBpm}. Do not change it based on mood or genre.`
      : 'Choose a musically appropriate BPM and return it as a number.';

    const systemPrompt = `You are SONGWRITER, a serious songwriting collaborator, arranger, and jazz-trained musician. You are NOT a chord-loop generator. Develop the user's raw lyrics into a complete, convincing ORIGINAL song arrangement that a human musician could rehearse.
...
Return ONLY valid JSON matching the schema.`;
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        text: { format: { type: 'json_schema', name: 'song_development', strict: true, schema: {
          type: 'object', additionalProperties: false,
          properties: {
            title:{type:'string'}, key:{type:'string'}, bpm:{type:'number'}, timeSignature:{type:'string'}, feel:{type:'string'},
            sections:{type:'array',minItems:1,items:{type:'object',additionalProperties:false,properties:{name:{type:'string'},lines:{type:'array',minItems:1,items:{type:'object',additionalProperties:false,properties:{text:{type:'string'},bars:{type:'array',minItems:1,maxItems:8,items:{type:'object',additionalProperties:false,properties:{chords:{type:'array',minItems:1,maxItems:4,items:{type:'string'}},lyricCue:{type:'string'}},required:['chords','lyricCue']}}},required:['text','bars']}}},required:['name','lines']}}},
    ...
    const text = (data.output || []).flatMap(item => item.content || []).filter(part => part.type === 'output_text' && typeof part.text === 'string').map(part => part.text).join('');
    if (!text) return res.status(502).json({ error: 'The AI returned no usable song data.' });
    let song;
    try { song = JSON.parse(text); } catch { return res.status(502).json({ error: 'The AI returned malformed song data. Try again.' }); }
